...
after_build do
    ...
    FileUtils.cp(File.expand_path('stylesheets/theme.less', source_dir), File.expand_path('build/stylesheets/theme.less', release_dir))
    FileUtils.cp(File.expand_path('stylesheets/elements.less', source_dir), File.expand_path('build/stylesheets/elements.less', release_dir))
...
helpers do
  ...
  def menu_item(id, caption = nil, icon=nil, clas=nil)
    icon_string = icon ? i(icon) + ' ' : ''
    return "<li class=\"#{active(id.downcase.gsub(' ', '-'))} #{clas}\"><a rel=\"tooltip\" data-placement=\"right\" data-original-title=\"#{caption || id}\" href=\"#{id.downcase.gsub(' ', '-')}.html\">#{icon_string}<span class=\"caption\">#{caption || id}</span></a></li>"
  end
